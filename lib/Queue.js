...
class Queue extends Array {
  ...
  /**
   * === GETTERS / SETTERS ===
   * $ variables points to those properties.
   */
  ...
  */
  ...
  push(...elements) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      elements[i].set("index", this.length + i);
    }
  ...
  shuffle() {
    ...
    /** Check for unneeded cases (empty, singletons). */
  ...
  sort(ascending = true) {
    ...
    const current = this[this.INDEX_POINTER];
  ...
  uniq() {
    ...
    let ids = this.map(e => e.get("id"));
    this.splice(0, this.length, ...this.filter((v, i, a) => {
